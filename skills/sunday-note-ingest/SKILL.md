...
1. 按固定 vault 布局读取用户指定的 Raw / Routine、已确认对话和相关 Wiki 页面；任务涉及跨来源评价、冲突处理或暂定结论时，按需读取根目录 `个人上下文.md` 的相关章节以决定综合方式，不用个人偏好改变来源事实；只在 Wiki 范围内查找同主题页面。
2. 分别判断知识增量和来源关系变化：只有材料新增事实、关系、边界或冲突，并能让现有页面无法回答的未来问题得到不同答案时才更新知识内容；新增可靠来源、替换错误来源、删除失效来源或修复来源链接时只维护来源关系。已有结论的推导、换写和解释性展开不算知识增量；两者均无变化时保持文件不变并说明原因。
...
5. 根据实际改动更新 Wiki header 和必要的 index。
...
- 已有页面的知识内容或来源关系发生实质变化时，将 `last_updated` 写为当天，`update_count` 每页每轮加 1，并在 `sources` 中汇总、去重本轮实际使用的来源。
- 来源关系维护包括新增可靠来源、替换错误来源、删除失效来源和修复来源链接；同步更新正文 wikilink 和 header `sources`。
...
- 知识内容和来源关系均无实质变化时不更新 header 或 index。
- 只有核心 Wiki 导航发生变化时才更新 index。
- Ingest 新建 Wiki 页面时，在维护日志中按现有格式写一条创建登记。

## 写入边界

- 用户明确要求把内容写入或更新 Wiki，即授权普通 Wiki 正文及其必要 header、index 和维护日志写入。
- 涉及删除、归档或未确认结论时，写入前请求确认。
