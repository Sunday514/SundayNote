...
description: 当任务涉及从本地 PDF 论文生成单篇论文摘要、解析论文正文和图表、整理论文贡献/方法/实验/局限，并把单篇论文总结保存到 Raw 层时使用。只处理单篇论文 source 摘要，不用于跨论文技术对比、Wiki 沉淀、项目调研报告或方案设计。
---
...
- 单篇论文总结写入 Raw：默认 `10_原始材料/论文/<paper-slug>/摘要.md`。
...
- `scripts/prepare_paper_summary.py`：创建 Raw 工作区、复制 PDF、写入 metadata、运行 docling 解析。
...
2. 读取 `.sunday-note-agent/config/sunday-note-vault.yaml`；缺少配置时使用默认 Raw 目录 `10_原始材料/论文`。
...
5. 读取脚本输出的 `paper_dir`，再读取：
   - `<paper_dir>/metadata.json`
   - `<paper_dir>/_work/parse/main_text.md`
   - `<paper_dir>/_work/parse/figure_index.md`
   - `<paper_dir>/_work/parse/figure_index.json`
   - `<paper_dir>/figures/`
...
```bash
conda run -n papers python .agents/skills/paper-summarizer/scripts/validate_summary.py <paper_dir>/摘要.md
conda run -n papers python .agents/skills/paper-summarizer/scripts/write_summary_status.py <paper_dir>
```
...
- 可加入“后续整合价值”小段，说明这篇论文是否值得进入跨论文对比或项目调研；这只是 Raw 层判断，不直接写 Wiki 或 Project。
